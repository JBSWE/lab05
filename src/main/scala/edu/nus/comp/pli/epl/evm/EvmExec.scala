...
case class EvmState(program: Seq[Instruction], opdStack: List[Result]) {
  ...
  def transition: EvmState = (program, opdStack) match {
    ...
    case (POW :: restProgram, ResultInt(n2) :: ResultInt(n1) :: restOpds) =>
      EvmState(restProgram, ResultInt(pow(n1,n2).toInt) :: restOpds)
    ...
    * */
    ...
    case (IFELSE ::restProgram,  (ResultInt(n2)) ::(ResultInt(n1)) :: (ResultBool(b3)) ::restOpds) => {
      if(ResultBool(b3) == ResultBool(true)) {
        EvmState(restProgram, (ResultInt(n1)):: restOpds)
      } else {
        EvmState(restProgram, (ResultInt(n2)):: restOpds)
      }
    }

    case (IFELSE ::restProgram,  (ResultBool(elseb)) ::(ResultBool(ifb)) :: (ResultBool(cond)) ::restOpds) => {
      if(ResultBool(cond) == ResultBool(true)) {
