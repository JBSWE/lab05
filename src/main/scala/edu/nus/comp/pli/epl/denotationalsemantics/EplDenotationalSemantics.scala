...
object EplDenotationalSemantics {
  private def compileHelper(expr: Expression): Seq[Instruction] = expr match {
    ...
    case Bin(Sub, left, right) => compileHelper(left) ++ compileHelper(right) :+ MINUS
    case Bin(Mul, left, right) => compileHelper(left) ++ compileHelper(right) :+ TIMES
    case Bin(Div, left, right) => compileHelper(left) ++ compileHelper(right) :+ DIV
    case Bin(Pow, left, right) => compileHelper(left) ++ compileHelper(right) :+ POW
    case Bin(And, left, right) =>  compileHelper(left) ++ compileHelper(right) :+ AND
    /*add your code for or*/
    case Bin(Or, left, right) => compileHelper(left) ++ compileHelper(right) :+ OR
    case Bin(Lt, left, right) => compileHelper(left) ++ compileHelper(right) :+ LT
    /*add your code for lte*/
    case Bin(Lte, left, right) => compileHelper(left) ++ compileHelper(right) :+ LE
    case Bin(Gt, left, right) =>  compileHelper(left) ++ compileHelper(right) :+ GT
    /*add your code for gte*/
    case Bin(Gte, left, right) => compileHelper(left) ++ compileHelper(right) :+ GE
    case Bin(Eq, left, right) =>  compileHelper(left) ++ compileHelper(right) :+ EQ
    /*add your code for NEq*/
    case Bin(NEq, left, right) => compileHelper(left) ++ compileHelper(right) :+ NEQ
